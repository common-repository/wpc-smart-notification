...
class notification {
	...
	function cart() {
		...
		$item['content'] = apply_filters( 'WPCSN/cart/item_content', sprintf( $link, sprintf( /* translators: cart */ esc_html__( 'Your cart: %s', 'wpc-smart-notification' ), initialization::get_cart() ) ) );
	...
	function new_orders() {
		...
		$args = [
			...
			'post_status'    => apply_filters( 'wpcsn_new_order_statuses', array_keys( wc_get_order_statuses() ) ),
		...
		$the_query = new WP_Query( apply_filters( 'wpcsn_new_order_args', $args ) );
		...
		if ( $the_query->have_posts() ) {
			while ( $the_query->have_posts() ) {
				...
				if ( $time_ago < HOUR_IN_SECONDS ) {
					$time_str = sprintf( /* translators: minute */ _n( 'about %d minute ago.', 'about %d minutes ago.', round( $time_ago / MINUTE_IN_SECONDS ), 'wpc-smart-notification' ), round( $time_ago / MINUTE_IN_SECONDS ) );
				} elseif ( $time_ago < DAY_IN_SECONDS ) {
					$time_str = sprintf( /* translators: hour */ _n( 'about %d hour ago.', 'about %d hours ago.', round( $time_ago / HOUR_IN_SECONDS ), 'wpc-smart-notification' ), round( $time_ago / HOUR_IN_SECONDS ) );
				} else {
					$time_str = sprintf( /* translators: day */ _n( 'about %d day ago.', 'about %d days ago.', round( $time_ago / DAY_IN_SECONDS ), 'wpc-smart-notification' ), round( $time_ago / DAY_IN_SECONDS ) );
				}
				...
				$item['title']     = apply_filters( 'WPCSN/new_orders/item_title', sprintf( /* translators: name & address */ esc_html__( '%1$s from %2$s purchased a', 'wpc-smart-notification' ), '<strong>' . $_name . '</strong>', '<strong>' . $_address . '</strong>' ), $_name, $_address, $order );
	...
	function virtual_orders() {
		...
		while ( $index < $number ) {
			...
			$item['title']     = apply_filters( 'WPCSN/virtual_orders/item_title', sprintf( /* translators: name & address */ esc_html__( '%1$s from %2$s purchased a', 'wpc-smart-notification' ), '<strong>' . $name . '</strong>', '<strong>' . $address . '</strong>' ), $name, $address, $data );
	...
	function on_sale_products() {
		...
		foreach ( $product_ids as $product_id ) {
			...
			$item['title']     = apply_filters( 'WPCSN/on_sale_products/item_title', sprintf( /* translators: product */ esc_html__( '%s is on-sale. Hurry up!', 'wpc-smart-notification' ), sprintf( $link, $product->get_permalink(), $product->get_name() ) ), $product, $product_ids );
	...
	function low_stock_products() {
		...
		if ( $the_query->have_posts() ) {
			while ( $the_query->have_posts() ) {
				...
				$item['title']     = apply_filters( 'WPCSN/low_stock_products/item_title', sprintf( /* translators: product & stock */ esc_html__( '%1$s has %2$d left only. Don\'t miss it!', 'wpc-smart-notification' ), sprintf( $link, $product->get_permalink(), $product->get_name() ), $product->get_stock_quantity() ), $product );
	...
	function related_products() {
		...
		foreach ( $related_product_ids as $product_id ) {
			...
			$item['title']     = apply_filters( 'WPCSN/related_products/item_title', sprintf( /* translators: product */ esc_html__( 'You may also like %s', 'wpc-smart-notification' ), sprintf( $link, $product->get_permalink(), $product->get_name() ) ), $product );
	...
	function viewing() {
		...
		if ( $product ) {
			...
			$viewing = sprintf( /* translators: counter */ esc_attr__( '%s people are viewing this product now.', 'wpc-smart-notification' ), rand( $min, $max ) );
